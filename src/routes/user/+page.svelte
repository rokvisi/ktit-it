<script type="ts">
    ...
    $: {
        if (items) {
            const itemsMatchingName = items.filter((item: Item) =>
                item.name.toLowerCase().includes(textSearch.toLowerCase())
            );
            const itemsMatchingDescription = items.filter((item: Item) =>
                item.description
                    .toLowerCase()
                    .includes(textSearch.toLowerCase())
            );
            const itemsMatchingGroup = items.filter((item: Item) =>
                item.groups.some((group) =>
                    group.toLowerCase().includes(textSearch.toLowerCase())
                )
            );
            ...
            for (const item of itemsMatchingName) {
                if (
                    filteredItems.findIndex(
                        (unique_item) => item.name === unique_item.name
                    ) === -1
                ) {
...
{:else}
    <input
        class="w-full mb-4 border rounded p-4"
        type="text"
        placeholder="Ieškoti..."
    ...
    <div class="flex flex-col gap-4 mb-10">
        {#each filteredItems as item (item.id)}
            <div
                ...
                on:click={() => goto(`/user/${item.id}`)}
            >
                <img
                    class="aspect-auto w-40"
                    src={item.images[0]}
                    alt="product"
                />
